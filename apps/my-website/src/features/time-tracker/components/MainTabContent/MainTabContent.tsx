    ...
    <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
      <div className="card bg-base-200 shadow-lg">
        <div className="card-body">
          <h2 className="card-title mb-4 text-xl">{t("form.addRecord")}</h2>
      ...
      <div className="card bg-base-200 shadow-lg">
        <div className="card-body">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="card-title text-xl">{t("records.recentRecords")}</h2>
            <button className="btn btn-sm btn-outline" onClick={() => setShowAllRecords(!showAllRecords)}>
              {showAllRecords ? t("records.showLess") : t("records.showAll")}
            </button>
          </div>
          <TimeRecordsList
            isLoading={isLoading}
          ...
          {!showAllRecords && records.length > 5 && (
            <div className="text-base-content/60 mt-4 text-center text-sm">
              {t("records.moreRecords", { count: records.length - 5 })}
            </div>
          )}
        </div>
      </div>
